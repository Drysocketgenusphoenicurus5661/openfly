...
from openfly.interfaces import Decision, MarketObservation, Prediction
...
class FixedDecoder:
    ...
    def predict(self, counts: np.ndarray, brain, observation: MarketObservation | None = None) -> Prediction:
        d = self.decode(counts, brain)
        side = Decision(d["side"])
        in_position = bool(observation is not None and observation.position_lots != 0)
        decision = side
        if side == Decision.ENTER and in_position:
            decision = Decision.HOLD
        elif side == Decision.EXIT and not in_position:
            decision = Decision.HOLD
