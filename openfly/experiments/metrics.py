...
def trade_stats(trades) -> dict:
    ...
    if n == 0:
        return {
            ...
            "mean_target_pct": None, "mean_lock_after_pct": None,
        }
    ...
    return {
        ...
        "mean_target_pct": float(np.mean([t.target_pct for t in trades])),
        "mean_lock_after_pct": float(np.mean([t.lock_after_pct for t in trades])),
