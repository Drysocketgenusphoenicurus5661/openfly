...
Every encoder turns a MarketObservation into a Stimulus: one luminance in
...
from openfly.interfaces import MarketObservation, Stimulus
from openfly.sensory.eyemap import EyeMap, as_eye_map, resolve_eye_map
...
def _closes(observation: MarketObservation) -> np.ndarray:
    return np.array([b.close for b in observation.index_bars], dtype=np.float64)


def _hlc(observation: MarketObservation) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def vix_zscore(observation: MarketObservation, window: int = 20) -> float:
...
class ChartEncoder(_BaseEncoder):
    ...
    def render_chart(self, observation: MarketObservation) -> Image.Image:
    ...
    def encode(self, observation: MarketObservation, brain=None) -> Stimulus:
...
class BarsEncoder(_BaseEncoder):
    ...
    def column_values(self, observation: MarketObservation) -> tuple[np.ndarray, np.ndarray, float]:
...
class FeatureEncoder(_BaseEncoder):
    ...
    def features(self, observation: MarketObservation) -> np.ndarray:
