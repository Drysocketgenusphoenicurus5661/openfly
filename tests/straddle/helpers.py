...
def observation(when: datetime, index: float = 23350.0, vix: float = 12.1, premium: float | None = None, engine: StraddleEngine | None = None) -> MarketObservation:
...
def observe(engine, broker, when, decision, call, put, *, roi=0.82, strike=23350.0, expiry=EXPIRY, ctx=None, index=23350.0, vix=12.1, win=None):
    quote = sq(when, call, put, strike, expiry)
    obs = observation(when, index=index, vix=vix, premium=quote.combined_ltp, engine=engine)
