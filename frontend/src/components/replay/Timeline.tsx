// A strip across the trading day: straddle spans as bands, one coloured
// marker per action, and the playhead. Clicking anywhere seeks.
    ...
    <div className="space-y-2">
      ...
      >
        {spans.map((span) => (
          <div
            key={span.n}
            className="absolute top-1 h-5 rounded border border-action-enter/40 bg-action-enter/10 px-1 text-[10px] leading-5 text-foreground"
            style={{
        ...
        {events.map((s) => (
          <div
            ...
            title={`${formatTime(s.t)} ${actionStyle(s.action).label}`}
          />
        ))}
